package com.pig4cloud.pigx.common.poi.excel.imports;
...
import com.pig4cloud.pigx.common.poi.excel.imports.base.ImportBaseService;
import com.pig4cloud.pigx.common.poi.excel.imports.verifys.VerifyHandlerServer;
...
import com.pig4cloud.pigx.common.poi.util.ExcelUtil;
...
public class ExcelImportServer extends ImportBaseService {
	...
	public ExcelImportResult importExcelByIs(InputStream inputstream, Class<?> pojoClass, ImportParams params) throws Exception {
		...
		for (int i = 0; i < params.getSheetNum(); i++) {
			if (log.isDebugEnabled()) {
				log.debug(" start to read excel by is ,startTime is {}", new Date().getTime());
			}
			if (isXSSFWorkbook) {
			...
			if (log.isDebugEnabled()) {
				log.debug(" end to read excel by is ,endTime is {}", new Date().getTime());
			}
			result.addAll(importExcel(result, book.getSheetAt(i), pojoClass, params, pictures));
			if (log.isDebugEnabled()) {
				log.debug(" end to read excel list by pos ,endTime is {}", new Date().getTime());
			}
		}
