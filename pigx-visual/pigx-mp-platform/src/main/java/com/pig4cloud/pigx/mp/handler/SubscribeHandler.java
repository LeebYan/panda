...
public class SubscribeHandler extends AbstractHandler {
	...
	public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage,
		...
		try {
			...
			WxAccountFans wxAccountFans = wxAccountFansMapper.selectOne(Wrappers
					.<WxAccountFans>lambdaUpdate().eq(WxAccountFans::getOpenid, wxMessage.getFromUser()));
			...
			WxAccount wxAccount = wxAccountMapper.selectOne(Wrappers.<WxAccount>lambdaQuery()
					.eq(WxAccount::getAppid, WxMpContextHolder.getAppId()));
	...
	private WxMpXmlOutMessage handleSpecial(WxMpXmlMessage wxMessage, WxAccountFans fans) {
		//发送关注消息
		List<WxAutoReply> listWxAutoReply = wxAutoReplyService.list(Wrappers.<WxAutoReply>query().lambda()
		...
		WxMpXmlOutMessage wxMpXmlOutMessage = MsgHandler.getWxMpXmlOutMessage(wxMessage, listWxAutoReply, fans, msgMapper, wxAccount);
